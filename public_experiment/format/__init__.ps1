...
if ($true) {
    ...
    $filteredFiles
    | Join-String -sep "`n - " -SingleQuote FullName -op 'Filtered Imports: '
    | Write-Debug

    $sortedFiles = $filteredFiles | Sort-Object { @('Write-TextColor') -contains $_.BaseName } -Descending
    $sortedFiles | Join-String -sep "`n - " -SingleQuote FullName -op 'Sorted Imports: '
    | Write-Debug
    # } catch {
...
| ForEach-Object {
    ...
    $curFile | Join-String -op 'CurFile: ' -sep "`n-" BaseName # FullName
    | Write-Debug
    # $curFile | Join-String -op 'CurFile: ' "`n-" BaseName # FullName
    # | Write-Warning
    # are these safe? or will it alter where-object?
    ...
    if ($false) {
        "`n`nerror: {0}" -f @(
            $curFile
            | Join-String -op 'CurFile: ' -sep "`n-" BaseName -DoubleQuote # FullName
        )
