...
function String-NinReplaceFancy {
    <#
    .synopsis
        sugar for "$x | %{ $_ -replace $regex, $replacement }"
    .description
       .not working correctly, revert to <200853751cff40505f6daf06f6e3abaf4c90277e>
    .example
          .
    .outputs
          [string
    ...
    param(
        ...
        # replacement pattern, works with -Templates
        [parameter(ParameterSetName = 'BasicRegex', Position = 1)]
        [parameter(ParameterSetName = 'ReplaceTemplate', Position = 0)]
        [AllowEmptyString()]
    ...
    begin {
        # default config
        $orignalSubString = $DefaultSubstitutionString
        ...
        if ( $PSCmdlet.MyInvocation.BoundParameters.ContainsKey('SubstitutionString')) {
            # warning this doesn't set correctly
            $SubstitutionString = $orignalSubString
            Write-Debug 'override, using users value'
        }
